...
function DashBoardChart() {
  ...
  useEffect(() => {
    try {
      console.log(`${REACT_APP_API_URL}/api/NhanVien/dashboardchart?n=${7}`);
      axios
        .get(`${REACT_APP_API_URL}/api/NhanVien/dashboardchart?n=${7}`)
        .then((res) => {
    ...
    <div className={clsx(style.chartContainer)}>
      <div className={clsx(style.chart)}>
        <Chart >
          ...
          <ChartTitle font="20pt sans-serif" text={"DOANH THU CỬA HÀNG TRONG 7 NGÀY GẦN NHẤT"} color='#088699'
          />
          <ChartSeries>
