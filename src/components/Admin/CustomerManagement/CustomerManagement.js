...
function CustomerManagement(props) {
    ...
    const getAllSize = async () => {
        ...
        let url = `${REACT_APP_API_URL}/api/KhachHang/all`;
    ...
    const openDialogFnc = (paramViewMode) => {
        ...
        if (!selectedRecord.MA_KH && paramViewMode !== 'add') {
            return;
        }
    ...
    const deleteSize = () => {
        if (!selectedRecord.MA_KH) {
            return;
        }
        console.log(JSON.parse(localStorage.getItem('employee')).accessToken);
        console.log(`${process.env.REACT_APP_API_URL}/api/KhachHang/deactivate`)
        try {
            axios.put(`${process.env.REACT_APP_API_URL}/api/KhachHang/deactivate?customerId=${selectedRecord.MA_KH}`
                , {},
                {
                    headers: {
                        Authorization: 'Bearer ' + JSON.parse(localStorage.getItem('employee')).accessToken,
        ...
        <div className={clsx(style.cartManagement)}>
            ...
            <div className={clsx(style.toolBar)}>
                ...
                <button onClick={() => {
                    ...
                    setConfirmDialogTitle('Xác nhận vô hiệu hóa tài khoản của khách hàng ' + selectedRecord.HO_TEN);
            ...
            {openDialog && <CustomerEdit customerId={selectedRecord.MA_KH} viewMode={viewMode} rerender={toggleReRender} closeDialog={closeDialog} />}
        </div>
    );
}
