...
import style from './SizeManagement.module.css';
...
function SizeManagement(props) {
    ...
    const getAllSize = async () => {
        ...
        let url = `${REACT_APP_API_URL}/api/BangSize/all`;
    ...
    const openDialogFnc = (paramViewMode) => {
        ...
        if (!selectedRecord.MA_SIZE && paramViewMode !== 'add') {
            return;
        }
    ...
    const deleteSize = () => {
        if (!selectedRecord.MA_SIZE) {
            return;
        }
        console.log(`${process.env.REACT_APP_API_URL}/api/BangSize/delete`)
        try {
            axios.delete(`${process.env.REACT_APP_API_URL}/api/BangSize/delete?sizeId=${selectedRecord.MA_SIZE}`
                ,
                {
                    headers: {
    ...
    return (
        <div className={clsx(style.cartManagement)}>
            ...
            >
                <ColumnsDirective>
                    <ColumnDirective field='MA_SIZE' clipMode='EllipsisWithTooltip' headerTextAlign='Center' headerText='Mã Size' width='200' textAlign="Left" /*isPrimaryKey={true}*/ />
                    <ColumnDirective field='TEN_SIZE' clipMode='EllipsisWithTooltip' headerTextAlign='Center' headerText='Tên Size' width='200' textAlign="Left" />
            ...
            {openDialog && <SizeEdit sizeId={selectedRecord.MA_SIZE} viewMode={viewMode} rerender={toggleReRender} closeDialog={closeDialog} />}
        </div>
    );
}
